...
export default function InventoryPage() {
  ...
  return (
    <div>
      ...
      {!error && parseInt(summary.low_stock || 0) > 0 && (
      ...
      {/* Inventory Table or Error */}
      {error ? (
        <div className="empty-state">
          <div className="empty-state-icon"><AlertTriangle size={40} color="var(--color-danger)" /></div>
          <h3>Failed to load inventory</h3>
          <p className="text-muted mb-md">We couldn't reach the server. Please check your connection.</p>
          <button className="btn btn-primary" onClick={() => { setLoading(true); loadInventory(); }}>
             Retry Connection
          </button>
        </div>
      ) : (
